...
def index():
    ...
    rows = db.execute(
        "SELECT DISTINCT symbol, SUM(shares) FROM history WHERE id = :id GROUP BY symbol", id=session["user_id"])
    ...
    grandTotal = totPrice + cash[0]["cash"]

    return render_template("index.html", table=table, cash=cash[0]["cash"], grandTotal=grandTotal)
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO history (id, symbol, shares, price) VALUES (:id, :symbol, :shares, :price)",
                   id=session["user_id"], symbol=quote["symbol"], shares=int(shares), price=price)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if int(shares) > rows[0]["SUM(shares)"]:
            return apology("YOU HAVE ONLY {} SHARES OF THIS COMPANY!".format(rows[0]["SUM(shares)"]))

        # Records in history table
        quote = lookup(symbol)
        db.execute("INSERT INTO history (id, symbol, shares, price) VALUES (:id, :symbol, :shares, :price)",
                   id=session["user_id"], symbol=quote["symbol"], shares=-int(shares), price=quote["price"])

        # Updates cash in users table
        cash = db.execute(
            "SELECT cash FROM users WHERE id = :id", id=session["user_id"])
        updatedCash = cash[0]["cash"] + quote["price"] * int(shares)
        db.execute("UPDATE users SET cash = :cash WHERE id = :id",
...
def change():
    ...
    if request.method == "POST":
        ...
        if len(newPassword) < 8:
            return apology("THE PASSWORD MUST CONTAIN ATLEAST 8 CHARACTERS!")
        if not (re.search(r"[0-9]", newPassword) and re.search(r"[a-z]", newPassword) and re.search(r"[A-Z]", newPassword) and re.search(r"\W", newPassword)):
            return apology("THE PASSWORD MUST CONTAIN ATLEAST ONE OUT OF 0-9, a-z, A-Z AND SPECIAL CHARACTERS!")
